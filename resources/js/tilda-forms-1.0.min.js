...
return!1});$('body').addClass('t-body_popupshowed');var $jbankform=$('#formbanktransfer'),arErrors;if($jbankform.length>0){$jbankform.off('submit');
...
$jbankform.submit(function(e){
    ...
    console.log("BROWN POWER");
    if(arErrors&&arErrors.length>0){
        window.tildaForm.showErrors($jbankform,arErrors);
        return!1
    }
    $.ajax({type:"POST",url:'/feedback',data:$jbankform.serialize(),dataType:"json",success:function(json){$('body').removeClass('t-body_popupshowed');$jbankform.closest('.t-banktransfer').remove();if(!json){json={error:'Unknown error. Please reload page and try again later.'}}
//https://forms.tildacdn.com/payment/banktransfer/
if(json&&json.error){alert(json.error);return!1}
window.tildaForm.sendStatAndShowMessage($jform,arArgs,!0)},error:function(data){$('body').removeClass('t-body_popupshowed');$jbankform.remove();alert(data)},timeout:15*1000})
})

}}else{window.tildaForm.sendStatAndShowMessage($jform,arArgs,!0)}}};window.tildaForm.closeSuccessPopup=function(){var $popup=$('#tildaformsuccesspopup');if($popup.length>0){$('body').removeClass('t-body_success-popup-showed');if(/iPhone|iPad|iPod/i.test(navigator.userAgent)&&!window.MSStream){window.tildaForm.unlockBodyScroll()}
...
$formurl='/feedback'; //https://forms.tildacdn.com/procces/
$.ajax({
    type:"POST",
    url:$formurl,
    ...
    success:function(json){
        ...
        if(json&&json.error){
            ...
            var virtPrice=0;var virtProduct='';
            if(window.Tilda&&typeof Tilda.sendEventToStatistics=='function'){
                ...
                Tilda.sendEventToStatistics(virtPage,virtTitle,virtProduct,virtPrice);
                if(window.dataLayer){window.dataLayer.push({'event':'submit_'+$jform.attr('id')})}}else{if(typeof ga!='undefined'&&ga){if(window.mainTracker!='tilda'){ga('send',{'hitType':'pageview','page':virtPage,'title':virtTitle})}}
